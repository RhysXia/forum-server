...
public interface RoleApi {
    ...
    @GetMapping
    ResultVo<Page<Role>> findAll(@PageableDefault Pageable pageable);

    @ApiOperation(value = "查询指定用户的角色")
    @GetMapping(params = "userId")
    ResultVo<Page<Role>> findAllByUserId(@RequestParam("userId") Long userId,
                                         @PageableDefault Pageable pageable);


    @ApiOperation(value = "查询指定id角色")
    @GetMapping("/{id}")
    ResultVo<Role> findById(@PathVariable("id") Long id);


    @ApiOperation(value = "添加角色，返回id")
    @PostMapping
    ResultVo<Role> add(@RequestBody RoleDto roleDto);

    @ApiOperation(value = "更新角色非空字段")
    @PutMapping(value = "/{id}", params = "all=false")
    ResultVo<Role> updateSelectionById(@PathVariable("id") Long id,
